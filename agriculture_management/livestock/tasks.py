...
def check_for_due_health_events():
    ...
    schedules = frappe.get_all("Vaccination Schedule", fields=["name", "species"])

    for schedule_details in schedules:
        schedule = frappe.get_doc("Vaccination Schedule", schedule_details.name)
        
        # Find all active animals of the matching species
        active_animals = frappe.get_all("Livestock", 
            filters={"status": "Active", "species": schedule.species}, 
...
def check_for_pasture_moves():
    ...
    try:
        settings = frappe.get_doc("Livestock Settings")
        if not settings.alert_recipient:
            frappe.logger().warning("Livestock pasture move check skipped: 'Alert Recipient' not set in Livestock Settings.")
            return

        frappe.logger().info(f"Alert Recipient found: {settings.alert_recipient}")

        tomorrow = add_days(nowdate(), 1)
        active_plans = frappe.get_all("Grazing Plan", filters={"status": "Active"}, fields=["name", "livestock_group"])
            ...
            for rotation in plan_doc.pasture_rotation:
                if rotation.planned_move_in_date and getdate(rotation.planned_move_in_date) == getdate(tomorrow):
                    ...
                    if not frappe.db.exists("ToDo", {"reference_type": "Grazing Plan", "reference_name": plan_doc.name, "description": subject, "status": "Open"}):
                        ...
                        frappe.logger().info(f"Successfully created ToDo for: {subject}")
                        ...
                        try:
                            ...
                            frappe.logger().info(f"Attempting to enqueue notification: {notification_doc}")
                            enqueue_create_notification([settings.alert_recipient], notification_doc)
                            frappe.logger().info(f"Successfully enqueued notification for user {settings.alert_recipient}")

                        except Exception as e:
                            # If anything goes wrong inside this block, log it with a full traceback
                            frappe.log_error(title="Failed to Enqueue Notification", message=frappe.get_traceback())
                        # --- END OF BLOCK ---
                        
    except Exception as e:
        frappe.log_error(title="Critical Error in check_for_pasture_moves", message=frappe.get_traceback())
...
def check_withdrawal_periods():
    ...
    for record_data in records_to_update:
        try:
            ...
            frappe.logger().info(f"Updated Treatment Record {doc.name} to Completed.")
        except Exception as e:
            frappe.log_error(title=f"Failed to update Treatment Record {record_data.name}", message=frappe.get_traceback())
...
def process_device_logs():
    ...
    """
    unprocessed_logs = frappe.get_all("Device Integration Log", filters={"status": "Unprocessed"})
    ...
    for log_info in unprocessed_logs:
        ...
        try:
            ...
            livestock_name = frappe.db.get_value("Livestock", {"animal_id_tag_id": tag_id})
            if not livestock_name:
                raise ValueError(f"No livestock found with Tag ID {tag_id}")

            # Update the animal's weight history
            animal = frappe.get_doc("Livestock", livestock_name)
            ...
            log_doc.save(ignore_permissions=True)
            frappe.log_error(title=f"Error processing Device Log {log_doc.name}")
