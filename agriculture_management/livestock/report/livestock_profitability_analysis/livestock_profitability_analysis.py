...
def get_columns():
    ...
    return [
        {"label": _("ID"), "fieldname": "id", "fieldtype": "Link", "options": "Livestock", "width": 140},
        ...
        {"label": _("Health Cost"), "fieldname": "health_cost", "fieldtype": "Currency", "width": 120},
        {"label": _("Gross Expenses"), "fieldname": "gross_expenses", "fieldtype": "Currency", "width": 130},
        {"label": _("Product Revenue"), "fieldname": "product_revenue", "fieldtype": "Currency", "width": 130},
        {"label": _("Sale Revenue"), "fieldname": "sale_revenue", "fieldtype": "Currency", "width": 120},
...
def get_data(filters):
    ...
    for animal in animals_to_process:
        ...
        health_cost = gl_entries.get(cost_center, {}).get("Veterinary Medicine", 0)
        
        # Calculate revenue
        product_revenue = product_sales.get(cost_center, 0)
        sale_revenue, acquisition_cost = get_animal_sale_info(animal.name, animal.asset, from_date, to_date)
        
        # Sum up totals
        gross_expenses = (acquisition_cost if not sale_revenue else 0) + feed_cost + health_cost
...
def get_animals(filters):
    ...
    return frappe.get_all("Livestock", fields=["name", "asset"], filters=f"docstatus = 1 {conditions}")
...
def get_cost_centers_for_animals(animals):
    ...
    animal_groups = frappe.get_all("Livestock", filters={"name": ("in", [d.name for d in animals])}, fields=["name", "livestock_group"])
    group_names = [d.livestock_group for d in animal_groups if d.livestock_group]
    group_cost_centers = frappe.get_all("Livestock Group", filters={"name": ("in", group_names)}, fields=["name", "cost_center"])
...
def get_animal_sale_info(livestock_name, asset_name, from_date, to_date):
    ...
    acquisition_cost = 0
    if asset_name:
        acquisition_cost = frappe.db.get_value("Asset", asset_name, "gross_purchase_amount") or 0
